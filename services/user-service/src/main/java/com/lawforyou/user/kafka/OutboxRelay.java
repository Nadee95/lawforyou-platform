...
/**
 * Polls the {@code outbox_events} table for PENDING events and forwards
 * them to Kafka. Runs every second (configurable via {@code app.outbox.relay-delay-ms}).
 */
@Slf4j
@Component
...
public class OutboxRelay {
    ...
    @Transactional
    public void relay() {
        // Re-queue FAILED events that still have retries remaining (retryCount < MAX_RETRIES).
        // Events that exhausted all retries (retryCount >= MAX_RETRIES) are permanently FAILED
        // and will NOT be picked up here — they require manual intervention:
        ...
        outboxEventRepository.findRetryableFailedEvents(MAX_RETRIES)
                .forEach(e -> {
                    ...
                    log.info("OutboxRelay: re-queuing FAILED event {} (retryCount={})",
        ...
        for (OutboxEvent event : pending) {
            try {
                ...
                log.debug("OutboxRelay: published {} [{}]",
                ...
                if (retries >= MAX_RETRIES) {
                    ...
                    log.error("OutboxRelay: FAILED event {} after {} retries — cause: {}",
                            event.getId(), retries, ex.getCause() != null ? ex.getCause().getMessage() : ex.getMessage());
                } else {
