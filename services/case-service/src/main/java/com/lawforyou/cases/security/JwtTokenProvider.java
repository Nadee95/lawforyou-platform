...
/** Validates JWT tokens issued by user-service. No token generation in case-service. */
@Slf4j
@Component
...
public class JwtTokenProvider {
    ...
    /**
     * Generates a token — used only in integration tests to create signed JWTs
     * without running a full user-service instance.
     */
    ...
    public UUID getTenantId(String token) {
        return UUID.fromString((String) parseClaims(token).getPayload().get(CLAIM_TENANT_ID));
    }

    public String getUsername(String token) {
        return (String) parseClaims(token).getPayload().get(CLAIM_USERNAME);
    }
